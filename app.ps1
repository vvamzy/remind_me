...
$AppName = 'TodoTimer'
$StorageDir = Join-Path $PSScriptRoot 'data'
...
function New-TodoPanel($todo){
  ...
  $panel.Padding = '8'
  $panel.ColumnDefinitions.Add((New-Object Windows.Controls.ColumnDefinition)) | Out-Null
  $panel.ColumnDefinitions.Add((New-Object Windows.Controls.ColumnDefinition)) | Out-Null
  ($panel.ColumnDefinitions[0]).Width = 'Auto'
  ($panel.ColumnDefinitions[1]).Width = '*'
  ...
  $panel.Children.Add($left) | Out-Null
  ...
  $panel.Children.Add($right) | Out-Null
...
function Update-Panel($panel){
  ...
  if($t.done){
    ...
    $panel.Tag.Bar.Foreground = New-Brush 63 63 70
  } else {
    ...
    $r = Lerp 0 hazard:255 $frac
  }
}

# fix typo (lerp green->red)
Remove-Item Function:\Update-Panel -ErrorAction SilentlyContinue
function Update-Panel($panel){
  $t = $panel.Tag.Todo
  $now = [DateTimeOffset]::Now.ToUnixTimeMilliseconds()
  $elapsed = [math]::Max(0, [math]::Min($t.totalMs, $now - $t.createdAt))
